var Book = function (_title, _numOfChapters, _author, _numOfPages, _publisher, _numOfCopies) {
...
Box.prototype.createBook = function (_title, _numOfChapters, _author, _numOfPages, _publisher, _numOfCopies) {
    var index = this.findIndex(_title);
    if (index === -1) {

        var newBook = new Book(_title, _numOfChapters, _author, _numOfPages, _publisher, _numOfCopies);
...
Box.prototype.deleteBook = function (_title) {
    ...
    if (index !== -1) {
        ...
        if (this.content[index].numOfCopies === 0)
            this.content.splice(index, 1)
    }
    else
        throw new Error("Cant find book")
}
