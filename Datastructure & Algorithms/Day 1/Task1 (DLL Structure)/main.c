...
struct Employee *employeExist(int id);

// display all employees
void displayScreen();

// print specific employee
void printEmp(struct Employee* trav);
...
void searchForEmployee()
{
    ...
    scanf("%i", &id);
    printf("\n=======================================\n");
    struct Employee *pSearch = employeExist(id);
    if (pSearch != NULL)
    {
        printEmp(pSearch);
    }
...
void deleteNode(struct Employee *node)
{
    if (Head == Tail)
        Head = Tail = NULL;
    else if (Head == node)
